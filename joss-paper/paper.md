...
Development and dissemination of comprehensive standards have been driven by community organisations such as the International Neuroinformatics Coordinating Facility (INCF) [@Abrams:2022]. This includes adoption of the FAIR principles [@Abrams:2022; @Wilkinson:2016] ensuring data are findable, accessible, interoperable and reusable.  Important standardisation initiatives include the Brain Imaging Data Structure (BIDS) [@Gorgolewski:2016], a file, folder and metadata standard widely used in neuroimaging, and the open file format Neurodata Without Borders (NWB) [@Teeters:2015; @Rubel:2022]. These initiatives aim to achieve ‘full’ standardisation that enables automated analysis of machine-readable experimental datasets.

There is a rich tooling ecosystem for working with standardised data. NWB provides packages for the reading, writing and validation of NWB files [@pynwb:2025; @NWBinspector:2025v064], alongside infrastructure for visualising and sharing [@Magland:2025]. The BIDS community has developed packages for converting raw data to BIDS format, as well as reading, writing and validating BIDS formatted folders, files and metadata [@Yarkoni:2019; @Gorgolewski:2020], while DataLad [@Halchenko:2021] supports the transfer of large datasets within similar workflows. ezBIDS [@Levitas:2024] offers a web-based GUI for BIDS conversion and sharing, though it is tailored towards neuroimaging. While highly valuable, these tools generally require coding experience and a good understanding of the underlying data schemas, with functionality distributed across multiple packages.
...
NeuroBlueprint is primarily a folder-organisation standard and is therefore not, on its own, sufficient to ensure data are FAIR. Consistent folder structures and naming conventions improve the findability of datasets and lower the barrier to depositing data in shared repositories, facilitating accessibility. However, interoperability and reusability require standardised file formats and metadata. NeuroBlueprint is heavily inspired by BIDS in order to minimise redundancy between specifications and facilitate later transition to this more comprehensive schema. As a result, NeuroBlueprint provides a low-overhead starting point for standardised data management while remaining compatible with a future extension toward fully FAIR-compliant workflows.

![The NeuroBlueprint specification.  Raw data (i.e. as collected from acquisition machines) are organised hierarchically by subject, session, and datatype. Subject  and session names consist of key-value pairs. Only the `sub-` and `ses-`keys are required and others are optional. Acquired data are placed in a datatype folder, with valid datatype names defined in the specification. Derived data are stored in the top-level `derivatives` folder, and while not mandated, it is advised to organise these similarly to the `rawdata` directory.
...
NeuroBlueprint-formatted folder trees can be created for a given subject, session and datatype (e.g. ‘funcimg’ for functional imaging), with online validation to reduce the likelihood of errors in user input (\autoref{fig:datashuttle-tui-example}).
...
Datashuttle will continue to evolve alongside the NeuroBlueprint specification, implementing upcoming extensions as they emerge. While datashuttle does not currently support a metadata standard, implementation of the simple, lightweight metadata schema recommended by NeuroBlueprint for basic project documentation will be a key focus for future development. While full FAIR compliance is out of scope for NeuroBlueprint itself, datashuttle plans to facilitate transition toward more comprehensive schemas by supporting automated conversion of NeuroBlueprint projects to BIDS, including metadata conversion, alongside raw data file conversion to open formats such as NWB.
