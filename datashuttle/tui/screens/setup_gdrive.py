...
class SetupGdriveScreen(ModalScreen):
    """
    This dialog window handles the TUI equivalent of API's `setup_google_drive_connection`.
    If the config contains a  "gdrive_client_id", the user is prompted to enter a client secret.
    Then, the user is asked if the their machine has access to a browser. If yes, a
    google drive authentication page on the user's default browser opens up. Else,
    the user is asked to run an rclone command on a machine with access to a browser and
    input the config token generated by rclone.
    """
    ...
    def ask_user_for_gdrive_client_secret(self) -> None:
        """
        Asks the user for google drive client secret. Only called if
        the datashuttle config has a gdrive_client_id.
        """
        message = (
    ...
    def ask_user_for_browser(self) -> None:
        """
        Asks the user if their machine has access to a browser.
        """
        message = (
    ...
    def open_browser_and_setup_gdrive_connection(self) -> None:
        """
        This removes the "yes" and "no" buttons that were asked during the
        browser question and starts an asyncio task that sets up google drive
        connection and updates the UI with success/failure. The connection setup
        is asynchronous so that the user is able to cancel the setup if anything
        goes wrong without quitting datashuttle altogether.
        """
    ...
    def setup_gdrive_connection_using_config_token(self) -> None:
        """
        Disables the enter button and starts an asyncio task to setup
        google drive connection and update the UI with success/failure.
        """
    ...
    async def setup_gdrive_connection_and_update_ui(
        ...
        """
        This starts the google drive connection setup in a separate thread (required
        to have asynchronous processing) and awaits for its completion. After completion,
        it displays a success / failure screen.
        """
    ...
    def setup_gdrive_connection(
        ...
        """
        This function runs in a worker thread to setup google drive connection.
