...
def setup_rclone_config_for_gdrive(
    ...
    """
    RClone sets remote targets in a config file that are
    used at transfer. For Google Drive, this sets the root folder.
    The relative path is supplied later during transfer.
    ...
    gdrive_folder_id : str
        The Google Drive Folder ID to use as the root directory.
...
def setup_rclone_config_for_gdrive(
    ...
    """
    RClone sets remote targets in a config file that are
    used for Google Drive. The relative path is supplied at transfer time.
    ...
    log : bool
        Whether to log, if True logger must already be initialized.
    """
    call_rclone(
        f"config create "
        f"{rclone_config_name} "
...
def transfer_data(
    ...
    extra_arguments = handle_rclone_arguments(
        rclone_options, include_list, connection_method
    )

    if upload_or_download == "upload":
        output = call_rclone(
            f"{rclone_args('copy')} "
            f'"{local_filepath}" "{central_filepath}" {extra_arguments}',
            pipe_std=True,
        )
...
def perform_rclone_check(
    ...
    """
    Perform RClone check with AWS S3 and Google Drive support.
    """
    connection_method = cfg["connection_method"]
