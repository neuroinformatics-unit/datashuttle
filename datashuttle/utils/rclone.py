...
def setup_rclone_config_for_aws(
    ...
    """
    RClone sets remote targets in a config file that are
    used at transfer. For AWS S3, this contains the bucket name
    and optional region parameter. Credentials are typically
    obtained from environment variables or ~/.aws/credentials.
    ...
    log : whether to log, if True logger must already be initialised.
    """
    region_param = f"region {cfg['aws_region']}" if cfg["aws_region"] else ""
...
def setup_rclone_config_for_gdrive(
    ...
    """
    RClone sets remote targets in a config file that are
    used at transfer. For Google Drive, this contains the folder ID
    to use as the root. Google Drive setup requires interactive
    authentication which this function doesn't handle directly.
    ...
    if log:
        utils.log(
            f"To configure Google Drive, run the following in your terminal:\n\n"
            f"rclone config create {rclone_config_name} drive "
            f"root_folder_id {cfg['gdrive_folder_id']}\n\n"
            f"This will start an interactive setup process where you'll need to "
            f"authenticate via a web browser."
        )
...
def transfer_data(
    ...
    """Transfer data with enhanced debugging"""
    import os

    utils.log(f"Starting {upload_or_download} for {top_level_folder}")
    utils.log(f"Connection method: {cfg['connection_method']}")
    ...
    utils.log(f"Central path: {central_filepath}")
    utils.log(f"Include list: {include_list}")

    if upload_or_download == "download":
        os.makedirs(local_filepath, exist_ok=True)
        utils.log(f"Created directory: {local_filepath}")

    rclone_config_name = cfg.get_rclone_config_name()
    utils.log(f"Using rclone config: {rclone_config_name}")
    ...
    if upload_or_download == "upload":
        cmd = (
            f"{rclone_args('copy')} "
            f'"{local_filepath}" "{rclone_config_name}:'
    ...
    utils.log(f"Running rclone command: {cmd}")

    output = call_rclone(cmd, pipe_std=True)

    if output.returncode == 0:
        utils.log("Transfer command completed successfully")
    else:
        utils.log(f"Transfer command failed: {output.stderr.decode('utf-8')}")
