...
def preliminary_for_setup_without_browser(
    ...
    """Prepare the rclone configuration for Google Drive without a browser.
...
def prompt_and_get_config_token(
    ...
    """Return Google Drive config token.

    Ask the user to run a command, authenticate with Google Drive and input
    the `config_token` generated by rclone. The `config_token` is
