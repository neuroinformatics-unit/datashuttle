...
if __name__ == "main":
    ...
    if platform == "darwin":
        subprocess.Popen(["open", f"{dir_path}/{path_to_wezterm}"],
                         env=my_env)  # TODO: don't need `path_to_wezterm` anymore.
    elif platform == "win32":
        subprocess.Popen(
            f"{dir_path}/{path_to_wezterm}  start conda activate {my_env['CONDA_DEFAULT_ENV']} && python {dir_path}/tui/app.py",
            env=my_env)
    else:
        subprocess.Popen(
            f"chmod +x {dir_path}/_vendored/wezterm/linux/wezterm.AppImage")
        subprocess.Popen([f"{dir_path}/_vendored/wezterm/linux/wezterm.AppImage"],
                         env=my_env)
